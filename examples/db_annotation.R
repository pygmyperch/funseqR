...
annotate_blast_results <- function(con, blast_param_id, max_hits = 5, e_value_threshold = 1e-10, 
  ...
  # Check UniProt API connection
  if (!test_uniprot_connection()) {
    stop("Cannot connect to UniProt API. Please check your internet connection.")
  }
  
  # Set up progress bar for UniProt queries
  if (verbose) {
    pb <- progress::progress_bar$new(
      format = "Querying UniProt [:bar] :percent ETA: :eta",
      total = length(hit_accessions),
      clear = FALSE,
      width = 60
    )
  }
  
  # Query UniProt for each accession
  uniprot_data <- list()
  ...
  # Set up progress bar for annotation storage
  if (verbose) {
    pb <- progress::progress_bar$new(
      format = "Storing annotations [:bar] :percent ETA: :eta",
  ...
  # Process BLAST results and store annotations
  successful_annotations <- 0
  annotation_ids <- c()
  
  tryCatch({
    for (i in 1:nrow(blast_results)) {
      blast_result_id <- blast_results$blast_result_id[i]
      hit_accession <- blast_results$hit_accession[i]
      ...
      if (!is.null(annotation_id)) {
        successful_annotations <- successful_annotations + 1
        annotation_ids <- c(annotation_ids, annotation_id)
      }
    ...
    stop("Error annotating BLAST results: ", e$message)
  })
}
