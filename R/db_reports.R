...
generate_template_content <- function(project_info, template_type) {
  ...
  if (template_type == "detailed") {
    detailed_content <- c(
      ...
      "  ",
      "  # Apply consolidation for cleaner visualization",
      "  main_chroms <- get_main_chromosomes(con)",
      "  if (!is.null(main_chroms) && length(main_chroms) > 0) {",
      "    # Create temporary VCF data for consolidation",
      "    vcf_temp <- data.frame(chromosome = rep(chrom_dist$chromosome, chrom_dist$variant_count))",
      "    vcf_consolidated <- consolidate_scaffolds(vcf_temp, main_chroms, verbose = FALSE)",
      "    chrom_dist <- aggregate(variant_count ~ chromosome, ",
      "                           data = data.frame(chromosome = vcf_consolidated$chromosome, variant_count = 1), ",
      "                           FUN = sum)",
      "    chrom_dist <- chrom_dist[order(chrom_dist$variant_count, decreasing = TRUE), ]",
      "  }",
      "  ",
      "  if (nrow(chrom_dist) > 0) {",
      "    plot_title <- 'Variant Distribution by Chromosome'",
      "    if (!is.null(main_chroms) && length(main_chroms) > 0) {",
      "      plot_title <- paste0(plot_title, ' (Consolidated View)')",
      "    }",
      "    ",
